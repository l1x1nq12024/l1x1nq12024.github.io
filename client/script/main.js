	...
	update()
	{
		if(isdev==1)
		{
			while(oq.length)
			{
				...
				players[t.id].setDragX(500);
				this.physics.add.collider(players[t.id],ground,function(p,g){
					p.setDragX(500);
				});
				var tp=this.physics;
				...
				aliveid.forEach(function(i){
					if(i!=t.id)
					{
						tp.add.overlap(players[t.id].hand,players[i],bombed);
						tp.add.overlap(players[i].hand,players[t.id],bombed);
					}
				});
				players[t.id].hps=this.add.sprite(t.x,t.y-hpy,'hp');
				players[t.id].hps.scaleX=20;
				players[t.id].hp=100;
			}
			aliveid.forEach(function(i){
				var dx=0;
				...
				players[i].hps.scaleX=parseInt(players[i].hp/5);
				players[i].hps.x=players[i].x-(10-players[i].hp/10);
				players[i].hps.y=players[i].y-hpy;
				if(dx!=0)
...
{
	players[id].destroy();
	players[id].hand.destroy();
	players[id].hps.destroy();
	delete players[id];
...
function SetNew(id,str)
{
	...
	players[id].hps.x=obj.hpx;
	players[id].hps.y=obj.hpy;
	players[id].hps.scaleX=obj.hpsx;
}